...
const BooksComp = () => {
  const cartProducts = useSelector((state: AppState) => state.order.inCart).map(
    (p) => p._id
  )
    ...
    userService.getPublicContent().then(
      (response) => {
        ...
        if (arrayOfPath.length === 4) {
          const catagorisedProd = updatedProduct.filter((prod) =>
            prod.genres
              .toString()
              .toLowerCase()
              .includes(arrayOfPath[3].toLowerCase())
          )
            ...
            <>
              ...
              {updateContent.length === 0 ? (
                <p style={{ width: '500px', margin: '0 auto' }}>
                  Oops! No books found. Please try a different keyword
                </p>
              ) : (
                <ul
                  style={{
